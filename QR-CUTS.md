...
| Guest order approval via real-time manager dashboard with push alerts | `Restaurant.qrApprovalMode` + Orders **QR approval queue** (poll every 2s) with Approve/Reject → kitchen `PLACED` / `CANCELLED`; guest track shows “Awaiting approval” | Consistent with the polling decision — no separate push channel for approvals. |
| Waiter push notifications (mobile push / native app) for service requests and approvals | In-app **Service requests** inbox on `/tables` (poll every 2s) with Ack/Done; table modal can Reopen / Close QR sessions | No push infrastructure. Staff keep the floor view open on a POS/tablet during service. |
